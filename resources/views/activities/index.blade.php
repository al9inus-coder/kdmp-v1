...
<x-ui.workspace title="Master Kegiatan" description="Kelola data kegiatan anggaran per program.">
    <x-slot:actions>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center gap-3">
                <div class="flex items-center gap-2 flex-1 max-w-xl">
                    <div class="relative flex-1">
                        ...
                        <x-ui.input type="text" name="q" class="pl-9" placeholder="Cari kode / nama kegiatan" value="{{ $search }}" />
                    </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-slate-50 border-b border-slate-100">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Nama Kegiatan</th>
                        <th class="px-6 py-3.5 text-xs font-bold text-slate-500 uppercase tracking-wider">Program</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($activities as $activity)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            <td class="px-6 py-4 text-center text-slate-500 font-medium">{{ $activities->firstItem() + $loop->index }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 font-semibold text-slate-900">{{ $activity->nama }}</td>
                            <td class="px-6 py-4">
                                <span class="text-xs font-bold text-slate-500">{{ $activity->program->kode }}</span>
                                <p class="text-slate-700 mt-0.5">{{ $activity->program->nama }}</p>
                            </td>
                            <td class="px-6 py-4">
        ...
        @elseif($activities->isNotEmpty())
            <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                <p class="text-sm text-slate-500">
                    Menampilkan <span class="font-semibold text-slate-700">{{ $activities->count() }}</span> kegiatan
                </p>
            </div>
        @endif
    </x-ui.card>
