...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="{{ auth()->user()->hasRole('Staff') ? route('staf.packages.import') : route((auth()->user()->hasRole('Kabid') ? 'kabid.' : 'admin.') . 'packages.import.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm">
