...
<div class="space-y-6">
    ...
    {{-- Ambil riwayat import milik user ini --}}
    @php
        $batches = \App\Models\ImportBatch::with('fiscalYear')
    ...
    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($batches as $batch)
                        <tr class="hover:bg-slate-50 transition-colors cursor-pointer"
                            onclick="window.location='{{ route('staf.packages.import.show', $batch->id) }}'">
        ...
        @if($batches->isNotEmpty())
            <div class="p-4 border-t border-slate-100 bg-slate-50/50">
                <p class="text-sm text-slate-500">Menampilkan {{ $batches->count() }} riwayat import terakhir.</p>
            </div>
        @endif
    </div>
</div>
