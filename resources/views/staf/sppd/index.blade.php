        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 flex items-center gap-2">
                    ...
                    Daftar <span class="text-indigo-600">SPD</span>
                </h1>
                <p class="text-sm text-slate-500 mt-1">Ajukan dan pantau status persetujuan SPD Anda.</p>
            </div>
            @php
            ...
            <div class="flex items-end gap-1 px-3 pt-2 border-b border-slate-200 overflow-x-auto">
                ...
                @foreach ($spjTabs as $value => $tab)
                    @php
                        ...
                        $tabUrl = route(
                            'staf.sppd.index',
                            array_filter(array_merge(request()->except(['status', 'page']), ['status' => $value])),
                        );
                    @endphp
                    <a href="{{ $tabUrl }}"
                        class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold whitespace-nowrap rounded-t-xl border-b-2 transition-colors
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex flex-col items-center gap-1.5">
                                        ...
                                        @if ($to->status === \App\Models\TravelOrder::STATUS_APPROVED)
                                            ...
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold border {{ $spjMeta['badge'] }}">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm mt-1">Klik "Ajukan SPD" untuk membuat pengajuan pertama.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
