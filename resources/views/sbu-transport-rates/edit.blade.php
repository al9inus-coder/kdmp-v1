...
<x-ui.workspace title="Edit Biaya Transportasi" description="Perbarui standar biaya transportasi.">
    <x-slot:actions>
        <x-ui.button variant="outline" size="md" href="{{ route('admin.sbu-transport-rates.index') }}">
            <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i> Kembali
        </x-ui.button>
    </x-slot:actions>

    <form action="{{ route('admin.sbu-transport-rates.update', $sbuTransportRate) }}" method="POST">
        @csrf
        @method('PUT')
