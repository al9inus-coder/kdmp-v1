@props(['disabled' => false, 'readonly' => false, 'invalid' => false, 'required' => false])

@php
    $baseClass = 'block w-full rounded-md shadow-sm sm:text-sm transition-colors duration-200 ease-in-out px-3 py-2 ';
    $stateClass = $invalid 
        ? 'border-red-300 text-red-900 focus:ring-red-500 focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-offset-0 ' 
        : 'border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-0 bg-white ';
    ...
    // Select usually doesn't have a readonly attribute in standard HTML that works identically, but pointer-events-none can emulate it.
