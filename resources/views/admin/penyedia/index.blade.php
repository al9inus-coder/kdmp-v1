...
@php
    ...
    $filterUrl = fn($s) => route($rolePrefix . '.penyedia.index', array_filter([
        'status'     => $s,
...
<x-ui.workspace title="Pengadaan Penyedia" description="Ikuti tahapan workflow: persiapan, pemilihan penyedia, pelaksanaan, hingga pembayaran. Klik kartu tahapan untuk memfilter.">
    ...
    <x-ui.card padding="none">
        ...
        <div class="px-6 py-4 border-b border-slate-100">
            <form action="{{ route($rolePrefix . '.penyedia.index') }}" method="GET" class="w-full">
                @if($status)
                ...
                <x-ui.toolbar search="true" searchPlaceholder="Cari nama paket atau ID RUP...">
                    ...
                    @if(request()->hasAny(['search', 'status', 'program_id']))
                        <x-ui.button variant="ghost" size="sm" href="{{ route($rolePrefix . '.penyedia.index') }}">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($procurementPackages as $p)
                        @php
                            ...
                            $actionUrl = $package ? route('admin.procurement-packages.show', $package) : '#';
                            $s1 = 1; $s2 = 0; $s3 = 0; $s4 = 0;
                            ...
                            if($st === \App\Models\ProcurementPackage::WORKFLOW_PROVIDER_SELECTION) {
                                ...
                                $actionUrl = $package ? route('admin.procurement-packages.procurement-process.show', $package) : '#';
                                $s1 = 2; $s2 = 1;
                                ...
                                $actionUrl = $package ? route('admin.procurement-packages.execution.show', $package) : '#';
                                $s1 = 2; $s2 = 2; $s3 = 1;
                                ...
                                $actionUrl = $package ? route('admin.procurement-packages.payment', $package) : '#';
                                $s1 = 2; $s2 = 2; $s3 = 2; $s4 = 1;
