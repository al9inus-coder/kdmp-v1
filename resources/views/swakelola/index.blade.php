...
@php
    ...
    $ruangUrl = fn($r) => route('admin.swakelola.index', array_filter([
        'ruang'  => $r,
...
<x-ui.workspace title="Daftar Swakelola" description="Paket pengadaan jenis Swakelola — klik kartu jenis ruang untuk memfilter.">
    ...
    <x-ui.card padding="none" class="mb-6">
        <form method="GET" action="{{ route('admin.swakelola.index') }}" class="p-4 flex flex-col sm:flex-row gap-3">
            @if($ruangFilter)
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.swakelola.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($packages as $package)
                        <tr class="hover:bg-slate-50/80 transition-colors group cursor-pointer" onclick="window.location.href='{{ route((auth()->user()->hasRole('Kabid') ? 'kabid.' : 'admin.') . 'packages.show', $package) }}'">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-center">
                                    <a href="{{ route((auth()->user()->hasRole('Kabid') ? 'kabid.' : 'admin.') . 'packages.show', $package) }}" onclick="event.stopPropagation()"
