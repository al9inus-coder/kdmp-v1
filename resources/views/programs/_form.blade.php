@php
    $val = fn($field) => old($field, $program->{$field});
...
<section class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-2xl">
    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/60 flex items-center gap-3">
        ...
        <h3 class="text-sm font-bold text-slate-900">Informasi Program</h3>
    </div>
    <div class="p-6 space-y-4">
        <div>
            <label for="kode" class="block text-sm font-semibold text-slate-700 mb-1.5">
                Kode Program <span class="text-rose-500">*</span>
            </label>
            <x-ui.input type="text" name="kode" id="kode" maxlength="50" placeholder="Contoh: 1.02.01" :value="$val('kode')" :invalid="$hasError('kode')" required />
        ...
        <div>
            ...
            <x-ui.input type="text" name="nama" id="nama" maxlength="255" placeholder="Masukkan nama program" :value="$val('nama')" :invalid="$hasError('nama')" required />
...
<div class="flex flex-wrap items-center justify-end gap-3 mt-8 max-w-2xl">
    <x-ui.button variant="secondary" size="md" href="{{ route('admin.programs.index') }}">
