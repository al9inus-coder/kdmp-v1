...
<x-ui.workspace title="Master Program" description="Kelola data program anggaran.">
    <x-slot:actions>
        ...
        <x-ui.button variant="primary" size="md" href="{{ route('admin.programs.create') }}">
            <i data-lucide="plus" class="w-4 h-4 mr-2"></i> Tambah Program
        </x-ui.button>
    </x-slot:actions>

    <x-slot:toolbar>
        <form action="{{ route('admin.programs.index') }}" method="GET" class="w-full">
            <div class="flex flex-col lg:flex-row lg:items-center gap-3">
                <div class="flex items-center gap-2 flex-1 max-w-xl">
                    ...
                    @if($search || ($status !== null && $status !== ''))
                        <a href="{{ route('admin.programs.index') }}" class="inline-flex items-center p-2 text-slate-400 hover:text-rose-500 transition-colors shrink-0" title="Reset filter">
